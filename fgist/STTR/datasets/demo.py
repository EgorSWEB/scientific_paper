...
class CocoStyleTransfer(torchvision.datasets.CocoDetection):
    def __init__(self,content_folder, style_folder, target_folder, img_size):
#         super(CocoStyleTransfer, self).__init__(coco_img_folder, coco_ann_file)
        ...
        print(f"Коливество картинок контента: {len(content_images)}. Количество картинок стиля: {len(style_images)}.")
        
        self.images_pairs = []
        for x in content_images:
            c_name=os.path.basename(x).split(".")[0]
            for y in style_images:
                s_name=os.path.basename(y).split(".")[0]
                target_image_path = self.target_folder + '/' + c_name + '_stylized_' + s_name + '.png'
                if os.path.exists(target_image_path):
                    self.images_pairs.append([x,y])
...
#         noise_new_im+=+np.random.randn(new_w,new_h,3) * sigma / 255
    ...
    def __getitem__(self, pair_idx):
        ...
        # print('\033[92m', Tr.ToTensor()(target_img), '\033[0m')
        target_img = self.img_resize(target_img)
        ...
        if self._transforms is not None:
            ...
            target_img = self._transforms(target_img)
                
        return img, style_image, target, target_img
...
class CocoStyleTransferTest(torchvision.datasets.CocoDetection):
...
def build_test(image_set, args):

    if image_set=='val':
        content_folder = Path(args.in_content_folder_val)
        style_folder = Path(args.style_folder_val)
    else:
    ...
    dataset = CocoStyleTransferTest(content_folder, style_folder, args.img_size)
    return dataset
...
def build(image_set, args):
    ...
    assert content_folder.exists(), f'provided COCO path {content_folder} does not exist'
    assert style_folder.exists(), f'provided wikiart path {style_folder} does not exist'
    assert target_folder.exists(), f'provided target path {target_folder} does not exist'

    dataset = CocoStyleTransfer(content_folder, style_folder, target_folder, args.img_size)
    return dataset
